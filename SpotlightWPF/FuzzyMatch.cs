...
public static class FuzzyMatcher
{
	...
	public static bool FuzzyMatch(string stringToSearch, string pattern, out int outScore)
	{
		...
		// UPD: Apply matches score
		char[] distinct = stringToSearch.Distinct().ToArray();
		float matchesCount = 0;
		foreach (char chr in distinct)
		{
			matchesCount += pattern.Count(c => c == chr);
		}
		score += (int)Math.Round(matchesCount * matchesCountBonus);

		// UPD: Apply word matches score
		if(pattern.Contains(stringToSearch))
		{
			score += wordCamelMatchesCountBonus;
		}
		else if(pattern.ToLower().Contains(stringToSearch.ToLower()))
		{
			score += wordMatchesCountBonus;
		}
